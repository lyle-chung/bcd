...
int get_lowdata_from_flash(flash_s * flash, int active)
{
	...
	ret = read(fd, &(flash->data[0]), FLASH_SECTOR_SIZE - sizeof(unsigned long));
	if (ret < 0) {
	...
	//verify CRC check
	if (crc32(0, &(flash->data[0]), FLASH_SECTOR_SIZE - sizeof(unsigned long)) != flash->crc) {
...
int set_flash_with_lowdata(flash_s * flash, int active)
{
	...
	//calc CRC and write
	flash->crc = crc32(0, &(flash->data[0]), FLASH_SECTOR_SIZE - sizeof(unsigned long));
...
**********************************************/
static int cp_mtd2buf(unsigned char *buf)
{
    int fd; 
    ssize_t n;
...
static int cp_buf2mtd(unsigned char *buf)
{
    int fd; 
    int n;  
 ...
 */
static int envmatch (flash_s *env, unsigned char *s1, int i2)
{
    unsigned char c;
...
int get_bootcfg (char *name, char *val)
{
    ...
    cp_mtd2buf((unsigned char *)&bootenv);
...
int set_bootcfg (unsigned char *name, unsigned char *vals)
{
    flash_s bootenv;
    int len, oldval;
    unsigned char *env, *nxt = 0;
    ...
    for (env = bootenv.data; *env; env = nxt+1)
    {
        for (nxt = env; *nxt; ++nxt)
            ;
        if ((oldval = envmatch(&bootenv, name, (unsigned long)env - (unsigned long)bootenv.data)) >= 0)
        {
...
out:
    ...
    cp_buf2mtd((unsigned char *)&bootenv);
